...
int main(int argc, char *argv[]) {
    ...
    // --- Packed GF(2) Gauss-Jordan elimination ---
    // We pack each row of matrix_t (which has t_cols = rows bits) into 64-bit words
    // so XOR operations become fast word-level XORs. The algorithm finds pivot
    ...
    // allocate packed rows using std::vector for safer memory management
    std::vector<std::vector<uint64_t>> matrix_packed;
    matrix_packed.resize(t_rows);
    for (int rr = 0; rr < t_rows; rr++) {
        matrix_packed[rr].assign(W, 0ULL);
        for (int cc = 0; cc < t_cols; cc++) {
            if (matrix_t[rr][cc]) {
                int idx = cc / 64;
                int off = cc % 64;
                matrix_packed[rr][idx] |= (1ULL << off);
            }
        }
    }

    // We no longer need the int matrix or matrix_t; free them to reduce memory.
    ...
    // Gauss-Jordan elimination on packed matrix (vector-based)
    int i = 0; // current pivot row
    int j = 0; // current column (function index)
    std::vector<int> jb;            // list of pivot column indices
    std::vector<char> is_pivot(t_cols, 0); // marker for fast pivot membership

    while (i < t_rows && j < t_cols) {
        // Find a row `p` >= i such that bit j is 1
        int pivot_row = -1;
        int word_idx = j / 64;
        uint64_t bit_mask = 1ULL << (j % 64);
        for (int p = i; p < t_rows; p++) {
            if (matrix_packed[p][word_idx] & bit_mask) {
                pivot_row = p;
                break;
            }
        }
        ...
        if (pivot_row == -1) {
            // column j is all zeros -> dependent column (no pivot here)
            j++;
        } else {
            // record pivot column
            jb.push_back(j);
            ...
            if (pivot_row != i) {
                matrix_packed[i].swap(matrix_packed[pivot_row]);
            }

            // eliminate bit j from all other rows
            for (int k = 0; k < t_rows; k++) {
                if (k == i) continue;
                if (matrix_packed[k][word_idx] & bit_mask) {
                    // row k ^= row i (word-wise)
                    for (int w = word_idx; w < W; w++) {
                        matrix_packed[k][w] ^= matrix_packed[i][w];
                    }
                    // Note: we start from word_idx because lower words contain bits < j
                }
            }
            i++;
            j++;
    ...
    // remove duplicates using the fast pivot membership marker instead of std::find
    r = 0;
    for (int bits = 1; bits <= MAX_XOR_BITS; bits++) {
        for (pointer f : functions[bits]) {
            if (r < (int)is_pivot.size() && !is_pivot[r]) {
    ...
    // matrix_packed is vector-based; it will free automatically on scope exit
